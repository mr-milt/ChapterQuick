function updateValues() {
  try {
    ...
    chrome.storage.local.get(
      ["speed", "scrolling", "comments", "makeDark"],
      function (result) {
        if (chrome.runtime.lastError) {
          console.error(chrome.runtime.lastError);
          document.getElementById("speed-value").textContent =
            "Error loading speed";
          document.getElementById("scrolling-state").textContent =
            "Error loading scrolling state";
        } else {
          ...
          if (result.scrolling !== undefined) {
            document.getElementById("scrolling-state").textContent =
              result.scrolling ? "On" : "Off";
          } else {
            document.getElementById("scrolling-state").textContent = "Off";
          }
          if (result.comments !== undefined) {
            document.getElementById("comments-state").textContent =
              result.comments ? "Showing" : "Not Showing";
          } else {
            document.getElementById("comments-state").textContent = "Showing";
          } if (result.makeDark !== undefined) {
            document.getElementById("darkmode-state").textContent =
              result.makeDark ? "On" : "Off";
          } else {
